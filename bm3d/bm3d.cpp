...
,   vector<float> &img_basic
,   const unsigned width//图像宽
,   const unsigned height//图像高
,   const unsigned chnls//通道值
...
,   fftwf_plan *  plan_2d_inv
    ...
    auto pcs = std::chrono::high_resolution_clock::now();
    precompute_BM(patch_table, img_noisy, width, height, kHard, NHard, nHard, pHard, tauMatch);
    auto pce = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> duration_pc = pce - pcs;
    std::cout << "precompute bm 耗时: " << duration_pc.count() << " ms" << std::endl;
...
,   fftwf_plan *  plan_2d_inv
    ...
    //! Precompute Bloc-Matching
    vector<vector<unsigned> > patch_table;
    auto pcs1 = std::chrono::high_resolution_clock::now();
    precompute_BM(patch_table, img_basic, width, height, kWien, NWien, nWien, pWien, tauMatch);
    auto pce1 = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> duration_pc1 = pce1 - pcs1;
    std::cout << "precompute bm 耗时: " << duration_pc1.count() << " ms" << std::endl;   
    //! Preprocessing of Bior table
    vector<float> lpd, hpd, lpr, hpr;
    bior15_coef(lpd, hpd, lpr, hpr);
    ...
    for (unsigned ind_i = 0; ind_i < row_ind.size(); ind_i++)
    {
        ...
        for (unsigned ind_j = 0; ind_j < column_ind.size(); ind_j++)
        {
            ...
            // float* group_3D_est = sycl::malloc_shared<float>(chnls * nSx_r * kWien_2 , q);
            // float* group_3D_img = sycl::malloc_shared<float>(chnls * nSx_r * kWien_2 , q);

            //fill(group_3D_est , group_3D_est+chnls * nSx_r * kWien_2 , 0.0f);
            //fill(group_3D_img , group_3D_img+chnls * nSx_r * kWien_2 , 0.0f);
...
,   const unsigned i_max
    ...
    // sycl::queue q(sycl::gpu_selector_v);
    // float* img_gpu = sycl::malloc_shared<float>(img.size(), q);
    // for(int c=0;c<img.size();c++) img_gpu[c] = img[c];    
    ...
    //     float* vec_gpu = sycl::malloc_shared<float>(size , q);
    //     float* dct_gpu = sycl::malloc_shared<float>(size , q);

    //     q.parallel_for(chnls , [=](auto c){
    ...
    //                         vec_gpu[p * kHW + q + dc_p + (i * width + j) * kHW_2] =
    //                             img_gpu[dc + (i_r + i - nHW + p) * width + j + q];
    //     }).wait();

    //     //! Process of all DCTs
    //     fftwf_execute_r2r(*plan_1, vec_gpu, dct_gpu);
    ...
    //                         dct_gpu[dc_p + (i * width + j) * kHW_2 + k] * coef_norm[k];
    //     }
    ...
    //     //! Compute the new DCT
    //     float* vec = sycl::malloc_shared<float>(chnls * kHW_2 * step * width , q);
    //     float* dct = sycl::malloc_shared<float>(chnls * kHW_2 * step * width , q);
    ...
    //                                     img_gpu[(p + i + 2 * nHW + 1 - step + i_r - nHW)
    //                                                         * width + j + q + dc];
    //     }).wait();
...
,   const float    tauMatch
    ...
    vector<float> diff_table(width * height);//0
    vector<vector<float> > sum_table((nHW + 1) * Ns, vector<float> (width * height, 2 * threshold));//0   0
    ...
    //申请usm
    float *diff0_table=malloc_shared<float>(diff_table.size(),q);
    float *img0 = malloc_shared<float>(img.size(),q);
    float *sum0_table=malloc_shared<float>((nHW+1)*Ns*width*height,q);
    float *patch0_table=malloc_shared<float>(patch_table.size()*32,q);
    int *patch0_table_size=malloc_shared<int>(patch_table.size()*32,q);
    float *row0_ind=malloc_shared<float>(row_ind.size(),q);
    float *column0_ind=malloc_shared<float>(column_ind.size(),q);
    // float *value0=malloc_shared<float>(Ns)
    //float *table_distance_f=malloc_shared<float>(Ns*Ns,q);
    //unsigned *table_distance_u=malloc_shared<unsigned>(Ns*Ns,q);
    //std::cout<<"sum0_table size   = "<<(nHW+1)*Ns*width*height<<std::endl; //max = 166011375 pgm = 166020096
    //std::cout<<"patch0_table size = "<<patch_table.size()*32<<std::endl;// max= 295936  pgm =  9469952  
    unsigned sumtable_ncol=width*height;
    //拷贝数据
    for (unsigned i = 0; i < diff_table.size(); i++)
    {
        diff0_table[i]=diff_table[i];
    }
    for (unsigned j = 0; j < img.size(); j++)
    {
        img0[j]=img[j];
    }
    for (unsigned d=0;d < (nHW+1)*Ns*width*height;d++)
    {
        sum0_table[d]=2 * threshold;
    }
    for (unsigned k = 0; k <row_ind.size(); k++)
    {
        row0_ind[k]=row_ind[k];
    }
    for (unsigned i = 0; i < column_ind.size(); i++)
    {
        column0_ind[i]=column_ind[i];
    }
    ...
    // auto maxGmem = q.get_device().get_info<info::device::global_mem_size >();
    //   std::cout <<"global mem size = "<< maxGmem <<"\n";
    ...
    auto for1s = std::chrono::high_resolution_clock::now();
    q.parallel_for<class init_diff0>(range<3>((height - 2*nHW),(width - 2*nHW),nHW),[=](id<3> index){
            //auto di=item.get_global_id(0);
            auto i=index[0];
            auto j=index[1];
            auto dj=index[2];
            unsigned k = i * width + nHW;
            for(unsigned di=0;di<Ns;di++){
                const int dk = (int) (di * width + dj) - (int) nHW;
                diff0_table[k] = (img0[k + dk] - img0[k]) * (img0[k + dk] - img0[k]);
            }
    });
    q.wait();
    auto for1e = std::chrono::high_resolution_clock::now();

    // 计算时间间隔
    std::chrono::duration<double, std::milli> duration_for1 = for1e - for1s;

    // 将时间间隔以毫秒为单位输出
    std::cout << "for1: " << duration_for1.count() << " ms" << std::endl;
    
    auto for2s = std::chrono::high_resolution_clock::now();
    q.parallel_for(nHW*Ns,[=](id<1> index){
        auto di=index/nHW;
        auto dj=index%nHW;
            const int dk = (int) (di * width + dj) - (int) nHW;
            ...
            // //! Compute the sum for each patches, using the method of the integral images
             const unsigned dn = nHW * width + nHW;//恒定8208
            ...
            for (unsigned p = 0; p < kHW; p++)
            {
                ...
                for (unsigned q = 0; q < kHW; q++, pq++){
                    ...
                    value += diff0_table[pq];
                }
            }
    });
    q.wait();
    auto for2e = std::chrono::high_resolution_clock::now();

    // 计算时间间隔
    std::chrono::duration<double, std::milli> duration_for2 = for2e - for2s;

    // 将时间间隔以毫秒为单位输出
    std::cout << "for2: " << duration_for2.count() << " ms" << std::endl;
    
//     auto start0 = std::chrono::high_resolution_clock::now();
//     q.parallel_for(range<2>((nHW+1)*Ns,kHW),[=](id<2> index){
//         auto di=index[0]/Ns;
//         auto dj=index[0]%Ns;
//         auto p=index[1];
...
//         float value=0.0f;
//         for (unsigned q = 0; q < kHW; q++)
//            value += diff_table[pq+q];
//         sum0_table[ddk*sumtable_ncol+dn] = value;
//     });
//     q.parallel_for((range<2>((nHW+1)*Ns,width - 2*nHW-1)),[=](id<2> index){
//         auto di=index[0]/Ns;
//         auto dj=index[0]%Ns;
//         auto j= index[1]+nHW + 1;
//         const int dk = (int) (di * width + dj) - (int) nHW;
//         const unsigned ddk = di * Ns + dj;
//         const unsigned dn = nHW * width + nHW;
//         const unsigned ind = nHW * width + j - 1;
//         float sum = sum0_table[ddk*sumtable_ncol+ind];
//         for (unsigned p = 0; p < kHW; p++){
//             sum += diff0_table[ind + p * width + kHW] - diff0_table[ind + p * width];
//         }
//         sum0_table[ddk*sumtable_ncol+ind + 1] = sum;//if 
//     });
...
for (unsigned di = 0; di <= nHW; di++)
        for (unsigned dj = 0; dj < Ns; dj++)
        {
            ...
            {
                const unsigned ind = nHW * width + j - 1;// 16*512 + (16+1 -- 512-16 )-1     8208 -- 8687
                float sum = sum0_table[ddk*sumtable_ncol+ind];
                for (unsigned p = 0; p < kHW; p++)
                    sum += diff0_table[ind + p * width + kHW] - diff0_table[ind + p * width];
                sum0_table[ddk*sumtable_ncol+ind + 1] = sum;// 8209 --  8688
            }//if 
            
            //! General case
            for (unsigned i = nHW + 1; i < height - nHW; i++)
            {
                const unsigned ind = (i - 1) * width + nHW;// (16+ (0-479) )*512 + 16        8208 -- 261,649跨步512  
                float sum = sum0_table[ddk*sumtable_ncol+ind];//8208
                //! 1st column, left
                for (unsigned q = 0; q < kHW; q++)
                    sum += diff0_table[ind + kHW * width + q] - diff0_table[ind + q];
                sum0_table[ddk*sumtable_ncol+ind + width] = sum;// 8208+512 -- 跨步512  

                //! Other columns
                unsigned k = i * width + nHW + 1;// (16--511)*512 +16+1 = 8209 --   261,649  跨步512
                unsigned pq = (i + kHW - 1) * width + kHW - 1 + nHW + 1;
                for (unsigned j = nHW + 1; j < width - nHW; j++, k++, pq++)
                {   //8209 --   261,649  跨步512 
                    sum0_table[ddk*sumtable_ncol+k] =
                          sum0_table[ddk*sumtable_ncol+k - 1]//8209-1 --   8209+512-16-16-1=8688
                        + sum0_table[ddk*sumtable_ncol+k - width]//8209-512=7697  --   261,649-512  跨步512
                        - sum0_table[ddk*sumtable_ncol+k - 1 - width]//8209-1 --   261,649-512  跨步512
                        + diff0_table[pq]
                        - diff0_table[pq - kHW]
                        - diff0_table[pq - kHW * width]
                        + diff0_table[pq - kHW - kHW * width];
                }
            }
        }
    // q.parallel_for(range<3>((nHW+1)*Ns,(height - 2*nHW-1),(width - 2*nHW-1)),[=](id<3> idx){
    //     auto di=idx[0]/Ns;                                                                           
    //     auto dj=idx[0]%Ns;
    //     auto i=idx[1];
    //     auto k=idx[2];
    ...
    //             for (unsigned p = 0; p < kHW; p++)
    //                 sum += diff0_table[ind + p * width + kHW] - diff0_table[ind + p * width];
    //             sum0_table[ddk*sumtable_ncol+ind+1] = sum;
    //         }
    ...
    //         for (unsigned q = 0; q < kHW; q++)
    //             sum += diff0_table[ind + kHW * width + q] - diff0_table[ind + q];
    //         sum0_table[ddk*sumtable_ncol+ind + width] = sum;
    //         //! Other columns
    //         unsigned k0 = i * width + nHW + 1;
    //         unsigned pq = (i + kHW - 1) * width + kHW - 1 + nHW + 1;
    //             sum0_table[ddk*sumtable_ncol+k+k0] =
    //                  sum0_table[ddk*sumtable_ncol+k - 1+k0]
    //                 + sum0_table[ddk*sumtable_ncol+k - width+k0]
    //                 - sum0_table[ddk*sumtable_ncol+k - 1 - width+k0]
    //                 + diff0_table[pq]
    //                 - diff0_table[pq - kHW]
    //                 - diff0_table[pq - kHW * width]
    //                 + diff0_table[pq - kHW - kHW * width];
...
//     std::chrono::duration<double, std::milli> duration_ms0 = end0 - start0;
//     std::cout << "for0 :" << duration_ms0.count() << " ms" << std::endl;

    int len_1 = row_ind.size()*column_ind.size();
    int len_2 = row_ind.size();

    auto for4s = std::chrono::high_resolution_clock::now();
    q.parallel_for(len_1,[=](id<1> i){

        int ind_i = i / len_2;
        int ind_j = i % len_2;
            //! Initialization
            const unsigned k_r = row0_ind[ind_i] * width + column0_ind[ind_j];

            float table_distance_f[2500];
            unsigned table_distance_u[2500];

            for (unsigned i = 0; i < Ns*Ns; i++)
            {
                table_distance_f[i]=0;
                table_distance_u[i]=0;
            }
            ...
            for (int dj = -(int) nHW; dj <= (int) nHW; dj++)
            {
                for (int di = 0; di <= (int) nHW; di++){
                    if (sum0_table[(dj + nHW + di * Ns)*sumtable_ncol+k_r] < threshold){
                        table_distance_f[cnt]=sum0_table[(dj + nHW + di * Ns)*sumtable_ncol+k_r];
                        table_distance_u[cnt]=k_r + di * width + dj;
                        cnt++;
                    }
                }
                for (int di = - (int) nHW; di < 0; di++){
                    if (sum0_table[(-dj + nHW + (-di) * Ns)*sumtable_ncol+k_r] < threshold){
                        table_distance_f[cnt]=sum0_table[(-dj + nHW + (-di) * Ns)*sumtable_ncol+k_r + di * width + dj];
                        table_distance_u[cnt]=k_r + di * width + dj;
                        cnt++;
                    }
                }
            ...
            const unsigned nSx_r = (NHW > cnt ? r : NHW);

			//! To avoid problem
			if (nSx_r == 1 && cnt == 0)
			{
                table_distance_f[cnt]=0;
                table_distance_u[cnt]=k_r;
                cnt++;
			}
            for (unsigned n = 0; n < nSx_r; n++){
                patch0_table[k_r*32+n] = (table_distance_u[n]);
            }
               

			//! To avoid problem
			if (nSx_r == 1){
                patch0_table[k_r*32+1] = (table_distance_u[0]);
            }

            patch0_table_size[k_r] = cnt;

    }).wait();

    auto for4e = std::chrono::high_resolution_clock::now();
    // 计算时间间隔
    std::chrono::duration<double, std::milli> duration_for4 = for4e - for4s;
    // 将时间间隔以毫秒为单位输出
    std::cout << "for4: " << duration_for4.count() << " ms" << std::endl;

    auto for5s = std::chrono::high_resolution_clock::now();
    for (unsigned ind_i = 0; ind_i < row_ind.size(); ind_i++)
    {
        for (unsigned ind_j = 0; ind_j < column_ind.size(); ind_j++)
        {
            int k_r = row0_ind[ind_i] * width + column0_ind[ind_j];
            unsigned r = 1;
            while (r * 2 <= patch0_table_size[k_r]);
            r *= 2;
            const unsigned nSx_r = (NHW > patch0_table_size[k_r] ? r : NHW);
            for (unsigned n = 0; n < nSx_r; n++){
                patch_table[k_r].push_back(patch0_table[k_r*32+n]);

            }
            if (nSx_r == 1){
                patch_table[k_r].push_back(patch0_table[k_r*32+1]);

            }

        }
    }
    auto for5e = std::chrono::high_resolution_clock::now();
    // 计算时间间隔
    std::chrono::duration<double, std::milli> duration_for5 = for5e - for5s;
    // 将时间间隔以毫秒为单位输出
    std::cout << "for5 == copy patch_table back : " << duration_for5.count() << " ms" << std::endl;

    sycl::free(diff0_table,q);
    sycl::free(img0,q);
    sycl::free(sum0_table,q);
    sycl::free(patch0_table,q);
    sycl::free(row0_ind,q);
    sycl::free(column0_ind,q);
}
